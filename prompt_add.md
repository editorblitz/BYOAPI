...
User-Visible API Activity Log

To make the app’s behavior transparent for power users, the web UI includes a running log that shows what the API layer is doing for each operation, similar in concept and style to the progress log used in the standalone strip calculator tool. 

Log purpose

Provide real-time feedback while data is loading or calculations are running.

Show which NGI endpoints are being hit, which dates/locations are being processed, and when data is skipped (e.g. weekends/holidays or missing series).

Help users and support staff diagnose issues without exposing credentials.

Log behavior

Each main tool page (daily prices, spreads, strips, LNG flows, netbacks) includes a scrollable log panel (e.g. <div id="progressLog" class="progress-log">) styled similarly to the existing strip calculator:

Monospace font

Bounded height with vertical scrolling

New entries appended at the bottom

Frontend JavaScript exposes a helper like addProgressLog(message) that:

Appends a new <div> with the message to the log container.

Automatically scrolls the container to the newest entry, mirroring the behavior in the current strip calculator implementation. 

Security considerations for the log

The log may include endpoint names, query parameters (e.g. dates, hubs, terminal names), and high-level status messages, but must never display:

NGI email, API key, access token, or any part of those values.

Raw request headers or authorization strings.

Errors are reported in a generic yet useful way (“NGI API request failed with status 500”) without dumping full stack traces or sensitive response content into the browser.
