/**
 * Daily Spot Charts - Publication-ready NGI single-location daily price charts
...
const DailySpotCharts = {
    ...
    init: function() {
        ...
        this.bindEvents();
        this.setupLogToggle();
        this.log('Daily Spot Charts system initialized.');
    },
    ...
    handleGenerate: async function() {
        try {
            ...
            // Build URL with optional date params - use daily type with single location
            let url = `/api/quick-charts?type=daily&locations=${location}`;
            if (startDate && endDate) {
                url += `&start_date=${startDate}&end_date=${endDate}`;
                this.log(`Fetching daily chart for ${this.currentLocationName} from ${startDate} to ${endDate}...`);
            } else {
                this.log(`Fetching daily chart for ${this.currentLocationName} (last 12 months)...`);
            }
            ...
            // Convert multi-series format to single-series format
            const singleData = {
                dates: data.series[0].dates,
                averages: data.series[0].averages,
                location_name: data.series[0].location_name
            };
            ...
            if (singleData.dates && singleData.dates.length > 0) {
                document.getElementById('startDate').value = singleData.dates[0];
                document.getElementById('endDate').value = singleData.dates[singleData.dates.length - 1];
            }
        } catch (error) {
    ...
    downloadChart: function() {
        ...
        img.onload = () => {
            ...
            canvas.toBlob((blob) => {
                ...
                const filename = `${sanitizedName} Daily.webp`;
...
document.addEventListener('DOMContentLoaded', () => DailySpotCharts.init());
